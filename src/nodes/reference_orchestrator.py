...
def reference_send_payloads(state: WritingAgentState) -> list[ReferenceSendPayload]:
    ...
    return [
        ReferenceSendPayload(
            ...
            # 首写流水线需要前章规划摘要链；大纲是只读的小型结构，完整携带。
...
def make_reference_orchestrator_node(
    ...
    def node(state: ReferenceSendPayload) -> WritingAgentState:
        ...
        materials = [
            material_from_payload(material, chapter_id) for material in result["materials"]
        ]
        ...
        # Pregel 仅在整个超步结束后投递 Send；在此处同步衔接单章首写，
        # 才能让快章的写作与慢章的检索真正重叠。
        draft_state = cast(
            WritingAgentState,
            {key: value for key, value in state.items() if key != REFERENCE_CHAPTER_ID_KEY}
            | {"citation_library": materials},
        )
