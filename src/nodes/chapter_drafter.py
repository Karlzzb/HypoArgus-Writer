...
DRAFT_CHAPTER_ID_KEY = "draft_chapter_id"
...
def draft_send_payloads(state: WritingAgentState) -> list[dict[str, Any]]:
    ...
    return [
        {
            DRAFT_CHAPTER_ID_KEY: chapter.id,
            "outline": outline,
            "citation_library": [
                material
...
def make_chapter_drafter_node(
    ...
    def node(state: WritingAgentState) -> WritingAgentState:
        ...
        chapter_id = str(dict(state)[DRAFT_CHAPTER_ID_KEY])
