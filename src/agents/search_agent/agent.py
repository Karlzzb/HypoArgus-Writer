...
"""检索运行时全局预算的缺省值：覆盖默认六章，避免章节许可额外排队。

引擎内各来源通道仍由其各自的 provider 级并发与连接配置限流；本预算只限制
同时在跑的整章引擎调用，不能替代 provider 的安全边界。
"""
...
def make_search_agent(
    ...
    *,
    ...
    按环境变量 SEARCH_AGENT_MAX_CONCURRENT_CALLS（缺省 6）；该值是一个适配器
    实例（即服务运行时共享的检索子智能体）的全局整章检索预算。引擎内部
    provider 级限流保持独立生效。min_pass_per_chapter
    ...
    async def run(task: dict[str, Any]) -> dict[str, Any]:
        ...
        # 预算事件显式区分「等待全局预算」与「已在引擎内执行」：后者从
        # engine_call_start 开始计时，避免把章节许可排队误诊为下游 I/O。
        queued_at = time.monotonic()
        progress("engine_permit_queued", budget_limit=limit)
        try:
            await permit.acquire()
        except asyncio.CancelledError:
            progress("engine_permit_cancelled", budget_limit=limit)
            raise
        ...
        progress(
            "engine_call_start",
            hypothesis_count=len(task["hypotheses"]),
            budget_limit=limit,
            queue_wait_ms=queue_wait_ms,
        )
