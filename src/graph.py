...
检索阶段：framework_orchestrator 后的条件边为每个待写章节各发一个 Send——
有假说且素材未入库的章节进检索（reference_orchestrator 各分支完成本章检索并落
checkpoint 后，经条件 Send 进入管线首写节点；首写失败恢复时不重跑已完成检索），
无假说或素材已齐的章节跳过检索、直发管线首写节点，保证大纲全部章节都流经
汇合点（issue #80）。
素材和产物经 reducer 汇入主状态，全部章节完成后才汇合进入终审。
首写阶段：管线首写各分支并行写章（前文承接用规划摘要链），
...
def route_after_framework_orchestrator(state: WritingAgentState) -> str | list[Send]:
    """框架完成后的路由：为每个待写章节各发一个 Send 进入按章流水线。

    有假说且素材未入库的章节发往 reference_orchestrator（检索→首写流水线）；
    其余未写章节（无假说、或恢复续跑时素材已齐）跳过检索、直接发往管线
    首写节点——所有章节都流经 reference_join 汇合，「大纲全章到齐」的
    终审判定因此成立（issue #80：无假说章缺席流水线曾致图静默终结）。
    检索选章判定与 Send 载荷收敛于 reference_send_payloads 单一事实源。
    没有任何待写章节（全部已成稿的恢复场景）时直接落终审汇合路由。
    """
    reference_payloads = reference_send_payloads(state)
    retrieving = {payload["reference_chapter_id"] for payload in reference_payloads}
    drafted = {draft.chapter_id for draft in state.get("chapter_drafts", [])}
    direct_draft_ids = [
        chapter.id
        for chapter in state.get("outline", [])
        if chapter.id not in retrieving and chapter.id not in drafted
    ]
    sends = [
        Send("reference_orchestrator", payload) for payload in reference_payloads
    ] + [
        Send(PIPELINE_CHAPTER_DRAFTER_NODE, draft_send_payload_for_chapter(state, chapter_id))
        for chapter_id in direct_draft_ids
    ]
    if not sends:
