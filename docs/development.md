...
核心链路：章节 → 论点 → 假说三层论证体系，6 个主节点主路径流水线（首写阶段经 `Send` 并行扇出）+ 2 个子智能体（rewriter_loop 与 search_agent 均为真实实现），人工审阅无限迭代闭环。
...
| `src/agents/` | 子智能体：任务包契约（`contracts.py`）、`rewriter_loop/` 真实实现子包（编排、写作 LLM 注入点、真实适配器、风格校验器、随包风格指南；打桩同包共存、可显式注入）、`search_agent/` 真实检索适配层子包（契约映射、引擎运行时边界与假实现接缝、信号量限流；打桩同包共存、可显式注入） |
...
| 并发度 | `GRAPH_MAX_CONCURRENCY`(4)、`FRAMEWORK_MAX_CONCURRENT_CHAPTERS`(4)、`CITATION_MAX_CONCURRENT_CHAPTERS`(4)、`SEARCH_AGENT_MAX_CONCURRENT_CALLS`(2) | 依次为：图级并行分支上限（检索与首写 Send 扇出）、论证框架论点假说生成与引文语义核查的章节级 LLM 并发上限、检索适配层外部调用总并发（跨并行分支共享的信号量阈值） |
