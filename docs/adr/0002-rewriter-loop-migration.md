...
本次迁移把源仓库核心能力搬入本仓库并改造为完全符合本项目框架规范的真实现。
...
任务包与结果 dict 逐字遵守本项目既有契约（`agents/contracts.py`：mode/chapter_spec/materials/prev_chapter_summary/revision_directives/current_text → chapter_text/chapter_summary/self_check），零扩展。
...
包内 LLM 注入点 `WriterLlmClient` 协议（`writer_client.py`）扩为 draft/revise/audit 三方法，配确定性 `FakeWriterLlmClient`（按顺序消费的脚本化信封 + 调用记录），驱动全部编排与校验测试零网络零成本。
...
新建 `agents/rewriter_loop/` 子包：编排（`writer.py`）、LLM 注入点（`writer_client.py`）、真实适配器（`llm_adapter.py`）、风格校验器（`style_linter.py`）、风格指南文件（`style_guide.md`）。
...
- 包内注入点：`WriterLlmClient` 配 Fake 驱动编排测试（双模式、违规修一次、self_check 折叠）；风格校验器规则单测；adapter 单测（JSON 解析、退化重试、audit 降级）用 FakeLLM 预置文本应答。
- 契约注入点：子智能体契约测试对真实现保持同形态断言。
