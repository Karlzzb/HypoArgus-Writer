...
修订意见解析带定位增强（ADR-0009）：引文确定性子串匹配优先、未命中回退 LLM 判断到章；全局意见程序侧扇出为逐章指令；受影响章数超过大纲一半时携解析清单重新中断待 `confirm` 确认；意见含混或定位失败时回问用户、不猜测。
...
| `SEARCH_AGENT_MAX_CONCURRENT_CALLS` | 6 | 检索运行时全局预算：同一服务运行时共享的整章引擎调用上限；预算事件包含章节 `queue_wait_ms`，provider 级限流另行生效 |
...
| `POST /tasks/{thread_id}/review` | 提交人工审阅（`finalize` 定稿 / `revise` 携意见修订 / `confirm` 确认大扇出修订清单） |
