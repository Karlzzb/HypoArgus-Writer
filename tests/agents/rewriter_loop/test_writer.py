...
def test_写作编排_任务包章型进lint_维度章观点标题无表触发修一次(
    ...
    # 调研报告有不依赖素材的语义裁决项：素材池为空仍自审（修前 + 修后复检各一次）。
    fake = FakeWriterLlmClient(
...
def test_写作编排_调研报告无素材_语义裁决项仍自审且发事件(draft_task: dict[str, Any]) -> None:
    # ADR-0005 按文种分派：调研报告持有不依赖素材的语义裁决项（对比叙事/四步递进），
    # 素材池为空时自审不再跳过；人培方案的既有跳过行为由前面的用例守住。
    events, record_hook = _make_recorder()
    draft_task["doc_type"] = "调研报告"
    draft_task["doc_variant"] = None
    draft_task["materials"] = []
    clean_text = (
        "## 二、空间下沉：精准破局「基层人才荒」\n\n"
        "数据显示基层就业占比上升，较全省平均高出两个百分点。\n\n"
        "| 指标 | 数值 |\n| ---- | ---- |\n| 占比 | 41 |\n"
    )
    draft_task["chapter_spec"]["chapter_type"] = "维度章"
    fake = FakeWriterLlmClient(
        draft_script=[_envelope(clean_text)],
        audit_script=[AuditEnvelope()],
    )
    run = make_writer_run(fake, event_hook=record_hook)
    result = asyncio.run(run(draft_task))

    assert fake.audit_calls == [clean_text]
    assert result["self_check"] == {"citations_ok": True, "issues": []}
    progress = [payload for event_type, payload in events if event_type == SUBAGENT_PROGRESS]
    steps = [payload["step"] for payload in progress]
    # 自审有真实调用：audit 的 llm_call 事件对存在。
    assert steps == ["llm_call_start", "llm_call_end", "lint_done", "llm_call_start", "llm_call_end", "audit_done"]


def test_写作编排_语义裁决项违规_触发修一次且不判引用不通过(draft_task: dict[str, Any]) -> None:
    # 语义级裁决项（对比叙事）违规：折入统一违规清单并触发修一次；
    # 终态仍在则如实留 issues，但不属引用类 → citations_ok 保持 True。
    draft_task["doc_type"] = "调研报告"
    draft_task["doc_variant"] = None
    draft_task["materials"] = []
    clean_text = (
    ...
    draft_task["chapter_spec"]["chapter_type"] = "维度章"
    semantic_issue = AuditEnvelope(
        issues=[
            AuditIssue(
                item="comparison_narrative",
                label="对比叙事",
                excerpt="孤立数值断言句",
            )
        ]
    ...
    fake = FakeWriterLlmClient(
        draft_script=[_envelope(clean_text), _envelope(clean_text, "修后摘要")],
        audit_script=[semantic_issue, semantic_issue],
    )
    run = make_writer_run(fake)
    result = asyncio.run(run(draft_task))

    # 自审违规触发恰一次修订，违规清单携带裁决项规则名与中文名义。
    assert len(fake.draft_calls) == 2
    _, fix = fake.draft_calls[1]
    assert fix is not None
    assert any(v.rule == "self_audit_comparison_narrative" for v in fix)
    assert any("对比叙事" in v.message for v in fix)
    # 修后复审仍在 → 如实折叠；语义裁决项不属引用类，引用门禁不受影响。
    issues = result["self_check"]["issues"]
    assert any("self_audit_comparison_narrative" in issue for issue in issues)
    assert result["self_check"]["citations_ok"] is True
