...
def test_写作编排_draft检出lint违规_修一次且折叠修前结论(draft_task: dict[str, Any]) -> None:
    ...
    # 恰好一次 fix：第二次 draft 带非空 fix_violations；修后不复检（audit 只调一次、
    # 且审的是修前正文）。
    assert len(fake.draft_calls) == 2
    _, fix = fake.draft_calls[1]
    assert fix is not None and len(fix) > 0
    assert fake.audit_calls == [_MARKER_VIOLATION_TEXT]

    # 最终正文取第二次信封；self_check 折叠的是修前结论。
    assert result["chapter_text"] == fixed_text
    assert result["chapter_summary"] == "修后摘要"
    assert result["self_check"]["citations_ok"] is False
    assert any("unknown_material_marker" in issue for issue in result["self_check"]["issues"])


def test_写作编排_自审违规_触发修订且引用不通过(draft_task: dict[str, Any]) -> None:
    fake = FakeWriterLlmClient(
...
def test_写作编排_revise模式_收到任务包且无违规不修(draft_task: dict[str, Any]) -> None:
    ...
    # revise 模式走 client.revise，不触 draft；无违规不触发 fix。
...
def test_写作编排_进度事件_步骤序列与载荷完备(draft_task: dict[str, Any]) -> None:
    ...
    progress = [payload for event_type, payload in events if event_type == SUBAGENT_PROGRESS]
    steps = [payload["step"] for payload in progress]
    # draft/audit/fix 三次真实调用各包一对 llm_call_start/llm_call_end。
    assert steps == [
        "llm_call_start",
        "llm_call_end",
        "lint_done",
...
def test_写作配置_未设置环境变量_回落缺省值() -> None:
    assert load_writer_settings(env={}) == ("本科", "人才培养方案")


def test_写作配置_显式设置_读取环境变量值() -> None:
    env = {"REWRITER_LOOP_TIER": "高职", "REWRITER_LOOP_DOC_TYPE": "实训方案"}
    assert load_writer_settings(env=env) == ("高职", "实训方案")


def test_写作配置_非法层次_抛错并指明变量名() -> None:
