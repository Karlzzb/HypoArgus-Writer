...
from citation_validator import load_citation_max_retries, make_citation_validator_node
...
def _run(
    ...
    node = make_citation_validator_node(lambda unit: fake, max_retries=max_retries)
...
def test_环境变量缺省为2() -> None:
    assert load_citation_max_retries({}) == 2
    assert load_citation_max_retries({"CITATION_MAX_RETRIES": ""}) == 2
    assert load_citation_max_retries({"CITATION_MAX_RETRIES": "5"}) == 5
...
def test_环境变量非法值抛ValueError(raw: str) -> None:
    with pytest.raises(ValueError, match="CITATION_MAX_RETRIES"):
        load_citation_max_retries({"CITATION_MAX_RETRIES": raw})
