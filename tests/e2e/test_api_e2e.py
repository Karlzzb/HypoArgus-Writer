...
async def _read_sse(
    ...
    ended = False
    headers = (
        {"Last-Event-ID": last_event_id} if last_event_id is not None else None
    )
...
def test_端到端主干_混合修订两类分支经引文门禁定稿并渲染书目(backend: str):
    async def main(saver: Any) -> None:
        ...
        async with _client(app) as client:
            ...
            assert third["data"]["data"]["citation_warnings"] == []
...
def test_独立检索接口_请求体校验失败返回422():
    async def main() -> None:
        ...
        async with _client(app) as client:
            valid_hypotheses = [
                {"id": "h1", "text": "假说一", "refute_condition": ""}
            ]
            # 缺 hypotheses 字段。
            response = await client.post(
                "/retrieval", json={"chapter_id": "ch1"}
            )
            assert response.status_code == 422
            # 空假说列表。
            response = await client.post(
        ...
        app = _make_app(
            ...
            search_agent=lambda hook: make_search_agent(
                ...
                runtime=FakeSearchAgentRuntime(
                    side_effect=raise_configuration_error
                ),
            ),
        )
        async with _client(app) as client:
...
def test_世代失配重连立即收到reconcile_required后转实时():
    ...
    async def main() -> None:
        ...
        app1 = _make_app(
            FIRST_PASS_RESPONSES, checkpointer=saver, epoch="ep-old"
        )
        async with _client(app1) as client:
        ...
        async with _client(app2) as client:
            ...
            reader = asyncio.create_task(
                _read_sse(
                    ...
                    f"/tasks/{thread_id}/stream",
                    lambda fs: any(
                        f["event"] == "review_required" for f in fs
                    ),
    ...
    async def main() -> None:
        ...
        async with _client(app) as client:
            frames, _ = await _read_sse(
                ...
                "/graph_events",
                lambda fs: any(
                    f["event"] == "reconcile_required" for f in fs
                ),
                last_event_id="ep-graph-old-1",
            )
            rec = next(
                f for f in frames if f["event"] == "reconcile_required"
            )
            assert rec["data"]["reason"] == "epoch_mismatch"
