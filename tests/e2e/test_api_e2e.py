...
def _assert_material_contract(material: dict[str, Any]) -> None:
    ...
    assert (material["source_kind"] == "web") == (material["url"] is not None)
    assert set(material) == {
        "id",
        "hypothesis_id",
        "chapter_id",
...
def _assert_bibliography_entry_material_contract(entry: dict[str, Any]) -> None:
    material = entry["material"]
    assert entry["material_id"] == material["id"]
    _assert_material_contract(material)


def _assert_compatible_response_shape(left: Any, right: Any) -> None:
    """递归比较响应形状；空列表只声明该字段为数组。"""
    if isinstance(left, dict) and isinstance(right, dict):
        ...
        for key in left:
            if key == "source_ref":
                ...
                continue
            _assert_compatible_response_shape(left[key], right[key])
        return
    if isinstance(left, list) and isinstance(right, list):
        for items in (left, right):
            if items:
                first = items[0]
                for item in items[1:]:
                    _assert_compatible_response_shape(first, item)
        if left and right:
            _assert_compatible_response_shape(left[0], right[0])
        return
...
def _make_app(
    ...
    *,
    ...
    rewriter_loop_agent: Any | None = None,
    ...
    if rewriter_loop_agent is not None:
        subagent_kwargs["rewriter_loop"] = rewriter_loop_agent
    elif rewriter_stub:
        subagent_kwargs["rewriter_loop"] = make_stub_rewriter_loop()
...
def test_检索中断续跑_已完成章节零重复检索且事件配对产物与不中断路径等价():
    ...
    assert [draft.chapter_id for draft in snapshot.values["chapter_drafts"]] == ["ch1"]
    ...
    # 中断分支残链：适配层报告预算与失败终态，但异常令 subagent_end 不会伪造完成。
    assert [t for t, _ in ch2_events] == [
        SUBAGENT_START,
        SUBAGENT_PROGRESS,
        SUBAGENT_PROGRESS,
        SUBAGENT_PROGRESS,
    ...
    assert [p["step"] for _, p in ch2_events[1:]] == [
        "engine_permit_queued",
        "engine_permit_acquired",
        "engine_call_start",
        "engine_call_end",
    ]
    assert ch2_events[-1][1]["outcome"] == "error"
    ...
    async def main() -> None:
        ...
        async with _client(app) as client:
            ...
            assert response.status_code == 200
            assert response.json()["status"] == "ARTICLE_WRITING"
            ...
            assert [e["payload"]["step"] for e in progresses[:3]] == [
                "engine_permit_queued",
                "engine_permit_acquired",
                "engine_call_start",
            ]
...
def test_api回归_中文孤儿角标不会静默通过且书目不编造条目():
    """issue #52 回归：空素材池下模型输出 [素材id-1] 必须进审阅警告。"""

    async def empty_search_agent_run(task: dict[str, Any]) -> dict[str, Any]:
        del task
        return {"materials": []}

    async def orphan_writer_run(task: dict[str, Any]) -> dict[str, Any]:
        chapter_id = task["chapter_spec"]["id"]
        title = task["chapter_spec"]["title"]
        marker = " [素材id-1]" if chapter_id == "ch1" else ""
        return {
            "chapter_text": f"## {title}\n本章在无素材池时仍出现模型臆造角标。{marker}",
            "chapter_summary": f"{title}摘要。",
            "self_check": {"citations_ok": True, "issues": []},
            "doc_type": task["doc_type"],
            "doc_variant": task["doc_variant"],
        }
    ...
    async def main() -> None:
        app = _make_app(
            ...
            search_agent=SubagentAdapter("search_agent", empty_search_agent_run),
            rewriter_loop_agent=SubagentAdapter("rewriter_loop", orphan_writer_run),
            document_review_max_retries=0,
        )
        async with _client(app) as client:
            thread_id, _ = await _create_task(client, "sess-orphan-marker")
            business, _ = await _read_sse(
                client,
                f"/tasks/{thread_id}/stream",
                _stop_on_review_count(1),
            ...
            assert review_ready["data"]["data"]["citation_warning_count"] >= 1

            pack = (await client.get(f"/tasks/{thread_id}/review")).json()
            warnings_text = "\n".join(pack["citation_warnings"])
            assert "orphan_marker" in warnings_text
            assert "素材id-1" in warnings_text
            assert pack["citation_library"] == []

            rendered = (await client.get(f"/tasks/{thread_id}/bibliography")).json()
            assert rendered["bibliography"] == []
            texts = "\n".join(chapter["text"] for chapter in rendered["chapters"])
            assert "[素材id-1]" in texts
            ...
            assert ended
            finalized = (await client.get(f"/tasks/{thread_id}/bibliography")).json()
            assert finalized["bibliography"] == []
            finalized_texts = "\n".join(
                chapter["text"] for chapter in finalized["chapters"]
            )
            assert "[素材id-1]" in finalized_texts
...
def test_端到端主干_混合修订两类分支经引文门禁定稿并渲染书目(backend: str):
    async def main(saver: Any) -> None:
        ...
        async with _client(app) as client:
            ...
            # 主干链路完整：首写在 reference_orchestrator 的按章流水线中执行；
            # writing_orchestrator 只在修订轮运行。
            ...
            for entry in rendered["bibliography"]:
                ...
                _assert_bibliography_entry_material_contract(entry)
            ...
            assert [e["material"] for e in apa["bibliography"]] == [
                e["material"] for e in rendered["bibliography"]
            ]
...
def test_resume后审阅状态继续映射到同一批稳定素材身份():
    """issue #73：进程重启/resume 后 Material.id/source_ref 不漂移。"""
    saver = InMemorySaver(serde=checkpoint_serializer())
    thread_id = "resume-material-identity"
    _drive_to_review_gate(saver, thread_id)

    def material_identities(materials: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [
            {
                "id": material["id"],
    ...
    async def fetch_public_identity(app: FastAPI, session_id: str) -> dict[str, Any]:
        async with _client(app) as client:
            response = await client.post(
                f"/tasks/{thread_id}/resume", json={"session_id": session_id}
            )
            assert response.status_code == 200
            assert response.json()["status"] == "AWAIT_USER_REVIEW"
            ...
            bibliography = (
                await client.get(f"/tasks/{thread_id}/bibliography")
            ).json()
            product_materials = [
                material
            ...
            return {
                ...
                "review_identities": material_identities(pack["citation_library"]),
                "product_identities": material_identities(product_materials),
                "bibliography_ids": [
                    entry["material_id"] for entry in bibliography["bibliography"]
                ],
                "bibliography_refs": [
                    entry["material"]["source_ref"]
                    for entry in bibliography["bibliography"]
                ],
            }

    async def main() -> None:
        before = await fetch_public_identity(
            _make_app([], checkpointer=saver), "sess-resume-before"
        )
        after = await fetch_public_identity(
            _make_app([], checkpointer=saver), "sess-resume-after"
        )

        assert before == after
        assert before["review_identities"]
        assert before["review_identities"] == before["product_identities"]
        review_ids = [item["id"] for item in before["review_identities"]]
        assert before["bibliography_ids"] == review_ids
        assert before["bibliography_refs"] == [
            item["source_ref"] for item in before["review_identities"]
        ]
...
def test_审阅包_摘要事件与REST全文同源且review_required仅路由元数据():
    ...
    async def main() -> None:
        ...
        async with _client(app) as client:
            ...
            for material in pack["citation_library"]:
                _assert_material_contract(material)
...
def test_产物流_产物事件载荷与products快照逐字段对账():
    ...
    async def main() -> None:
        ...
        async with _client(app) as client:
            ...
            finalized = next(f for f in full if f["event"] == "finalized")
            assert "[1]" not in json.dumps(
                finalized["data"]["data"]["chapters"], ensure_ascii=False
            )
            assert any(
                MATERIAL_ID_PATTERN.search(chapter["text"])
                for chapter in finalized["data"]["data"]["chapters"]
            )
            rendered = (await client.get(f"/tasks/{thread_id}/bibliography")).json()
            rendered_text = " ".join(c["text"] for c in rendered["chapters"])
            assert "[1]" in rendered_text
            assert not MATERIAL_ID_PATTERN.search(rendered_text)
...
def test_mock档_thread_id带前缀且状态响应带mock标记():
    ...
    async def main() -> None:
        ...
        async with _client(app) as client:
            ...
            mock_bibliography = (
                await client.get(f"/tasks/{mock_tid}/bibliography")
            ).json()
            assert mock_bibliography["mock"] is True
            assert mock_bibliography["bibliography"]
            for entry in mock_bibliography["bibliography"]:
                _assert_bibliography_entry_material_contract(entry)

    asyncio.run(main())


def test_mock档与真实档_material审阅包书目响应形状一致():
    """issue #73：mock/real 在 Material、审阅包与 bibliography 契约上同形。"""

    async def collect(client: httpx.AsyncClient, thread_id: str) -> dict[str, Any]:
        products = (await client.get(f"/tasks/{thread_id}/products")).json()
        review = (await client.get(f"/tasks/{thread_id}/review")).json()
        bibliography = (await client.get(f"/tasks/{thread_id}/bibliography")).json()
        return {
            "products": products,
            "review": review,
    ...
    def assert_material_shapes(payload: dict[str, Any]) -> None:
        for chapter in payload["products"]["chapters"]:
            for material in chapter["materials"]:
                _assert_material_contract(material)
        for material in payload["review"]["citation_library"]:
            _assert_material_contract(material)
        for entry in payload["bibliography"]["bibliography"]:
            _assert_bibliography_entry_material_contract(entry)

    async def main() -> None:
        app = _make_app([*FIRST_PASS_RESPONSES, *REVISE_ROUND_RESPONSES])
        async with _client(app) as client:
            real_tid, _ = await _create_task(client, "sess-parity-real")
            mock_tid, _ = await _create_mock_task(client, "sess-parity-mock")

            await asyncio.gather(
                _read_sse(
                    client,
                    f"/tasks/{real_tid}/stream",
                    _stop_on_review_count(1),
                    last_event_id=FROM_START,
                ),
                _read_sse(
            ...
            real = await collect(client, real_tid)
            mock = await collect(client, mock_tid)
            assert_material_shapes(real)
            assert_material_shapes(mock)

            for key in ("products", "review", "bibliography"):
                _assert_compatible_response_shape(real[key], mock[key])
                assert real[key]["mock"] is False
                assert mock[key]["mock"] is True
