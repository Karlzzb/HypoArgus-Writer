...
def test_假LLM端到端_状态机按序流转至人工中断点():
    ...
    # chapter_drafter 并行扇出：2 章各一个任务、各回写一次单章更新
    #（两条更新同名同值，到达顺序不影响断言），章级产物落 checkpoint。
    assert observed == [
...
def _writer_keyed(chapter_titles: list[str]) -> dict[str, list[str]]:
    ...
    keyed: dict[str, list[str]] = {
        "【章节自审】": [AUDIT_EMPTY_RESPONSE] * len(chapter_titles)
    }
...
def test_写作自环中断恢复_已完成章节零重复调用且产物与不中断路径等价(backend: str):
    ...
    class _CrashOnSecondChapterWrite:
        ...
        写作注入点（LlmWriterClient）的退化重试会把每轮异常都打到这里，
        ...
        def invoke(self, messages: list[dict[str, str]]) -> str:
            text = joined_prompt(messages)
            if "- 标题：第二章" in text and "【章节自审】" not in text:
                self.crash_calls += 1
    ...
    with _checkpoint_backend(backend) as saver:
        ...
        # 写作注入点退化重试 3 轮全部命中注入异常、从未拿到信封后重抛。
        ...
        audit_prompts = [
            ...
            if "【章节自审】" in joined_prompt(messages)
        ]
        assert len(audit_prompts) == 1
        assert "m-ch2-p1-h1" in audit_prompts[0]
        assert "m-ch1-p1-h1" not in audit_prompts[0]
        # 前章衔接跨崩溃存续：恢复进程写第二章的提示词携带大纲里首章的
        # 规划摘要链（并行首写不依赖首章草稿，更不需要重写首章得来）。
        (ch2_write_prompt,) = [
            joined_prompt(messages)
            for messages in fake2.calls
            if "- 标题：第二章" in joined_prompt(messages)
            and "【章节自审】" not in joined_prompt(messages)
        ]
        assert "本章《第一章》论述：论点一。" in ch2_write_prompt
        ...
        # subagent_start、编排在写作调用前发 llm_call_start，异常沿注入点上抛后
