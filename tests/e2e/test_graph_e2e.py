...
def test_按章检索完成即启动首写_不等待最慢检索():
    """issue #78：快章的首写必须与慢章检索重叠，且最终仍完整汇合。"""
    import asyncio
    import time
    ...
    assert draft_started["ch1"] < retrieval_finished["ch2"]
    ...
    for update in graph.stream(
        ...
        for node_name, node_update in update.items():
            ...
            if node_update is None:
                # reference_join 是无操作汇合节点，不产生状态更新；Functional
                # API task 的更新也不对应主状态节点。
                assert node_name in {"reference_join", "retrieve"}
                continue
            if "status" not in node_update:
                assert node_name == "retrieve"
                continue
