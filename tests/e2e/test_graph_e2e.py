...
def test_按章检索落盘后启动首写且不提前终审():
    """issue #79：检索结果先落 checkpoint，再进入同章首写并完整汇合。"""
    import asyncio
    import time

    from agents.contracts import SubagentAdapter
    from agents.rewriter_loop import stub_rewriter_loop_run
    from agents.search_agent import stub_search_agent_run

    retrieval_finished: dict[str, float] = {}
    draft_started: dict[str, float] = {}

    async def delayed_search(task: dict) -> dict:
        chapter_id = task["chapter_id"]
        if chapter_id == "ch2":
            await asyncio.sleep(0.2)
        result = await stub_search_agent_run(task)
        retrieval_finished[chapter_id] = time.monotonic()
        return result

    async def recording_drafter(task: dict) -> dict:
        draft_started[task["chapter_spec"]["id"]] = time.monotonic()
        return await stub_rewriter_loop_run(task)

    graph, _, config = _build(
        FIRST_PASS_RESPONSES,
        search_agent=SubagentAdapter("search_agent", delayed_search),
        rewriter_loop=SubagentAdapter("rewriter_loop", recording_drafter),
    )
    result = graph.invoke(initial_state("意图", "身份", "trace-pipeline-timing"), config)

    assert set(draft_started) == {"ch1", "ch2"}
    assert draft_started["ch1"] > retrieval_finished["ch1"]
    assert draft_started["ch2"] > retrieval_finished["ch2"]
    assert [draft.chapter_id for draft in result["chapter_drafts"]] == ["ch1", "ch2"]
    assert result["status"] == WorkflowStatus.AWAIT_USER_REVIEW


def test_六章检索首写管线_执行器并发四不自死锁():
    """issue #79：章节数大于执行器并发时，检索管线不能因嵌套 task 自死锁。"""
    import asyncio
    import signal
    ...
    async def delayed_search(task: dict) -> dict:
        search_calls.append(task["chapter_id"])
        await asyncio.sleep(0.02)
        return await stub_search_agent_run(task)

    @contextmanager
    def invoke_timeout(seconds: float):
        def _raise_timeout(_signum: int, _frame: object) -> None:
            raise TimeoutError("6 章检索首写管线疑似执行器自死锁")
    ...
    graph, _, config = _build(
        [*framework_responses, *([SEMANTIC_PASS] * chapter_count), DOCUMENT_REVIEW_PASS],
        keyed=keyed,
        search_agent=SubagentAdapter("search_agent", delayed_search),
        rewriter_loop=SubagentAdapter("rewriter_loop", stub_rewriter_loop_run),
    )
    config["max_concurrency"] = 4

    with invoke_timeout(5):
        result = graph.invoke(initial_state("意图", "身份", "trace-six"), config)

    assert sorted(search_calls) == [f"ch{i}" for i in range(1, chapter_count + 1)]
    assert [draft.chapter_id for draft in result["chapter_drafts"]] == [
        f"ch{i}" for i in range(1, chapter_count + 1)
    ]
    assert result["status"] == WorkflowStatus.AWAIT_USER_REVIEW
...
def test_假LLM端到端_状态机按序流转至人工中断点():
    ...
    for update in graph.stream(
        ...
        for node_name, node_update in update.items():
            ...
            if node_update is None:
                ...
                continue
            if "status" not in node_update:
                assert node_name == "reference_orchestrator"
                continue
    ...
    assert observed == [
        ...
        ("reference_orchestrator", WorkflowStatus.REFERENCE_FETCHING),
        (PIPELINE_CHAPTER_DRAFTER_NODE, WorkflowStatus.ARTICLE_WRITING),
        (PIPELINE_CHAPTER_DRAFTER_NODE, WorkflowStatus.ARTICLE_WRITING),
...
def test_写作自环中断恢复_已完成章节零重复调用且产物与不中断路径等价(backend: str):
    ...
    def _recording_search(calls: list[str]) -> SubagentAdapter:
        async def _run(task: dict) -> dict:
            calls.append(task["chapter_id"])
            return await stub_search_agent_run(task)

        return SubagentAdapter("search_agent", _run)
    ...
    with _checkpoint_backend(backend) as saver:
        ...
        graph = build_graph(
            ...
            checkpointer=saver,
            search_agent=_recording_search(search_before),
        ...
        # 死亡现场：按章流水线中 ch1 已完成的首写写入被保留；失败的 ch2
        # 管线分支待恢复。其检索 task 已持久化，恢复不可重复调用外部检索。
        snapshot = graph.get_state(config)
        assert snapshot.next == (PIPELINE_CHAPTER_DRAFTER_NODE,)
        ...
        assert sorted(search_before) == ["ch1", "ch2"]
        ...
        graph2 = build_graph(
            ...
            checkpointer=saver,
            search_agent=_recording_search(search_after),
        ...
        assert search_after == [], "已持久化的检索 task 在恢复时不得重复外部调用"
...
def test_rewriter任务包prev_chapter_summary含多个前章摘要链():
    ...
    framework_3ch = [
        ...
        '{"chapter_index": 3, "points": [{"text": "论点三"}]}]',
    ]
    hypotheses_3ch = {
        "待发散的论点：论点一": ['[{"text": "假说一", "refute_condition": "出现公开反例即证伪", "angle": "假设", "evidence_retrievable": true}]'],
        "待发散的论点：论点二": ['[{"text": "假说二", "refute_condition": "出现公开反例即证伪", "angle": "预言", "evidence_retrievable": true}]'],
        "待发散的论点：论点三": ['[{"text": "假说三", "refute_condition": "出现公开反例即证伪", "angle": "边界条件", "evidence_retrievable": true}]'],
    }
