...
def test_写作自环中断恢复_已完成章节零重复调用且产物与不中断路径等价(backend: str):
    ...
    with _checkpoint_backend(backend) as saver:
        ...
        assert snapshot.next == ("chapter_drafter",)
...
def test_修订超步中断恢复_评审后改写崩溃则该章评审与改写整体重跑():
    ...
    修订超步现为「评审 + 改写」两次子智能体调用。故障注入在改写侧：ch2 的
    定向改写（rewriter mode=revise）抛致命异常——此时该章评审已完成，模拟
    进程死于修订超步中途；ch1 的修订超步已按超步落 checkpoint。断言：同
    thread_id 恢复后 ch2 从头重跑整个修订超步（评审与改写都重来，绝无跳过
    评审只补改写的半程续跑），已完成的 ch1 修订零重复子智能体调用。
    """
    from agents.chapter_reviewer import stub_chapter_reviewer_run
    from agents.contracts import SubagentAdapter
    from agents.rewriter_loop import stub_rewriter_loop_run
    ...
    # 一轮意见落两章各一条纯改写指令：ch1 超步先执行，ch2 超步中途崩溃。
    directive_response = json.dumps(
        [
            {
    ...
    # 节点重放、解析调用重复执行一次，故解析应答备两份。
    fake = FakeLLM(
        [*FIRST_PASS_RESPONSES, directive_response, directive_response],
    ...
    # 死亡现场：ch2 修订超步的评审虽已跑完，但超步崩溃后写入整体丢弃——
    # ch1 超步产物已落 checkpoint（草稿改写完成、指令队列只剩 ch2），
    # 待执行任务仍是 writing_orchestrator，不留 ch2 半成品。
    assert ("ch2", "revise") in rv_before  # 崩溃前该章评审确已执行。
    assert rw_before[-1] == ("ch2", "revise")
    snapshot = graph.get_state(config)
    ...
    # 超步原子性：恢复进程把 ch2 的修订超步从头重跑——评审与改写各恰一次；
